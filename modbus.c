...
void print_emerson_data ( emerson_data *edata ) {
    fprintf( stdout, "%li, %0.3f, %0.3f, %3.1f, %0.3f, %0.3f, %0.3f, %0.3f\n",
            edata->t, edata->cltp, edata->cltt,
            edata->hltp, edata->hltt,
            edata->mflowdv, edata->mflowv, edata->mflowt
    );
}
...
int insert_emerson_data( emerson_data *edata ) {
    ...
    if ( edata->hltp < 900000 ) {
        sprintf( qry, "INSERT INTO sensor_reports ( date_recorded, system_name, parameter_name, parameter_value, parameter_units, reported_by) VALUES ( %li, \"%s\", \"%s\", %8.3f, \"%s\", \"%s\" )", edata->t, "HLT", "Volume", edata->hltp, "InchesH2O", "Emerson Smart Gateway" );
        rtn = db_query(qry);
        if( rtn != 0 )
            return(rtn);
    }
...
void get_gateway_time( modbus_t *mb, char* date_string ) {
    ...
    if ( rc == -1 ) {
        ...
        sprintf( date_string, "%04i-%02i-%02i %02i:%02i:%02i",
                data[0],
                data[1],
                data[2],
                data[3],
...
int get_emerson_data( emerson_data *edata ) {
    uint16_t        tab_reg[32];
    int             rc = 0;
//    modbus_t        *mb = modbus_new_tcp( "192.168.1.24", 502 );
    modbus_t        *mb = modbus_new_tcp( "10.0.1.201", 502 );
...
//    modbus_set_debug( mb, TRUE );
    ...
    while ( rc >= 0 ) {
        rc = modbus_read_input_registers( mb, 0, 14, tab_reg );
        if ( rc == -1 ) {
            ...
            print_emerson_data( edata);
            insert_emerson_data( edata );
            get_gateway_time( mb, date_string );
            fprintf( log_file, "%s, %li, %0.3f, %0.3f, %3.1f, %0.3f, %0.3f, %0.3f, %0.3f\n",
                    date_string,
                    edata->t, edata->cltp, edata->cltt, edata->hltp, edata->hltt,
                    edata->mflowdv, edata->mflowv, edata->mflowt );
            fflush( log_file );
        }
        sleep(5);
...
void parse_emerson_data( uint16_t data[32], emerson_data *edata ) {
    ...
    edata->hltp     = modbus_get_float_badc(&data[4]);
    edata->hltt     = modbus_get_float_badc(&data[6]);
    edata->mflowdv  = modbus_get_float_badc(&data[8]);
    edata->mflowv   = modbus_get_float_badc(&data[10]);
    edata->mflowdv  = modbus_get_float_badc(&data[12]);
    edata->mflowt   = modbus_get_float_badc(&data[14]);
}
...
int read_and_insert_file() {
    ...
    while( !feof( infile) ) {
        ...
        if (buf[0] != '#') {
            n++;
            sscanf( buf, "%li, %f, %f, %f, %f, %f, %f, %f\n", &edata->t, &edata->cltp, &edata->cltt, &edata->hltp, &edata->hltt, &edata->mflowdv, &edata->mflowv, &edata->mflowt );
            rtn = insert_emerson_data(edata);
            if( rtn !=0 ) {
                fprintf( stderr, "Error when trying to insert emerson data.\n" );
                return(rtn);
